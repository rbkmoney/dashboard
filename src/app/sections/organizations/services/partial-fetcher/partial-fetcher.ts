...
/**
 * TODO: Temporary, while not implement https://github.com/rbkmoney/swag-organizations/pull/17
 */
export abstract class PartialFetcher<R, P> {
    private action$ = new Subject<FetchAction<P>>();

    readonly fetchResultChanges$: Observable<{ results: R[]; hasMore: boolean; continuationToken: string }>;
    ...
    constructor(debounceActionTime: number = 300) {
        ...
        this.searchResult$ = this.fetchResultChanges$.pipe(pluck('results'), shareReplay(SHARE_REPLAY_CONF));
