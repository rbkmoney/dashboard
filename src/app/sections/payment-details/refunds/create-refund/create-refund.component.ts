...
export class CreateRefundComponent implements OnInit {
    ...
    ngOnInit() {
        this.createRefundService.getAccount(this.createRefundData.shopID).pipe(tap((account) => {
    ...
    confirm() {
        ...
        this.createRefundService.createRefund(this.createRefundData.invoiceID, this.createRefundData.paymentID, params).pipe(take(1)).subscribe(_ => {
            this.dialogRef.close();
        });
    }
    ...
    onCheckboxChange(value: boolean) {
        ...
        if (value) {
            this.form.addControl('amount', this.fb.control('', [
                Validators.required,
                Validators.pattern(/^\d+([\,\.]\d{1,2})?$/),
                Validators.min(0.01)
                // TODO: max = payment amount
            ]));
        } else {
